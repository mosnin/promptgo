...
/** The most recent updated date across a set of tools. */
function newestUpdate(list: { updated: string }[], fallback: string) {
  const timestamps = list
    .map((tool) => new Date(tool.updated).getTime())
 ...
 * did change.
 *
 * Listing pages inherit the newest update among the tools they list, which is
 * exactly when their content actually changes.
 */
export default function sitemap(): MetadataRoute.Sitemap {
  const catalogueUpdated = newestUpdate(tools, STATIC_PAGE_REVISION);
