...
import { IdeToolByline, IdeToolPlatformNote } from "./IdeToolTrust";
import { IdeToolCard } from "./IdeToolCard";
import { toIdeToolCardData } from "@/lib/ide-tool-card";
import { getIdeToolCategory } from "@/lib/ide-tool-categories";
import { getRelatedIdeTools } from "@/lib/ide-tools";
...
import type { RegisteredIdeTool } from "@/lib/ide-tool-types";

/** The single page template every builder tool renders through, the ide-tools equivalent of `SkillShell`. */
export function IdeToolShell({ tool }: { tool: RegisteredIdeTool }) {
  const category = getIdeToolCategory(tool.category);
  const related = getRelatedIdeTools(tool, 6);
  const accent = category?.accent ?? "var(--color-signal)";

  const crumbs = [
    { name: "Home", href: "/" },
    { name: "Builder Tools", href: "/ide-tools" },
    { name: category?.name ?? "Builder Tools", href: `/ide-tools/${tool.category}` },
  ...
  return (
    <>
      ...
      <div className="shell py-10 sm:py-12">
        ...
        <Reveal distance={16}>
          <IdeEditor slug={tool.slug} starterFiles={tool.files} accent={accent} />
        </Reveal>
      ...
      <div className="shell pb-16">
        <div className="grid gap-12 xl:grid-cols-[minmax(0,1fr)_15rem]">
          <article className="min-w-0 max-w-3xl">
            ...
            {category && <IdeToolPlatformNote tool={tool} category={category} />}
              ...
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-[1.5rem] font-semibold tracking-[-0.025em] text-ink">
                    More {category?.name.toLowerCase() ?? "builder tools"}
                  </h2>
                </div>
                <a
                  href={`/ide-tools/${tool.category}`}
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {related.map((item, index) => (
                <Reveal key={item.slug} delay={index * 0.045} distance={14}>
                  <IdeToolCard tool={toIdeToolCardData(item)} accent={accent} index={index} />
                </Reveal>
              ))}
            </div>
          </div>
